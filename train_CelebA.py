...
#         loss_posterior += K.square(posterior[:,r]-1./k)
    ...
    return K.mean( 0. * loss_pred + 0. *loss_prior + 0.* loss_posterior 
                  +  0.0*loss_posterior_anti+1.0*loss_prior_anti+ 0. *loss_mean_posterior_prior
                 + 0.0 * loss_diff_pred)
...
#         loss_posterior += K.square(posterior[:,r]-1./k)
    ...
    return K.mean( 1. * loss_pred + 0. *loss_prior + 0.* loss_posterior 
                  +  1.0*loss_posterior_anti+0.0*loss_prior_anti+ 0. *loss_mean_posterior_prior
                 + 1.0 * loss_diff_pred)
...
#                 print 'i,j,lambda_mat[j,i]',i,j,lambda_mat[j,i]
 ...
 def train():   
    ...
    train_X, train_y_a, test_X, test_y_a, train_y_c, test_y_c, ATTR_NUM, CLASS_NUM, input_shape,data_name,lambda_mat,prior_list = gen_raw_data(['Eyeglasses'])
